    ...
    pub fn new() -> Self {
        Self {
            meta: ScriptMetadata {
                ...
                description: "Detects common web server vulnerabilities and misconfigurations".to_string(),
...
impl Script for HttpVulnsScript {
    ...
    fn run(&self, ctx: &ScriptContext) -> Result<ScriptResult, String> {
        ...
        if body_lower.contains("index of /") || body_lower.contains("directory listing") {
            result.add_finding(
                Finding::new(FindingType::Misconfiguration, "Directory Listing Enabled")
                    .with_description("Web server has directory listing enabled, exposing file structure")
            ...
            if body_lower.contains(pattern) {
                result.add_finding(
                    Finding::new(FindingType::InfoLeak, &format!("{} Disclosed", error_name))
                        ...
                        .with_severity(FindingSeverity::Medium)
                        .with_remediation("Implement custom error handling to hide database errors"),
                );
                break;
            }
        }
        ...
        if body_lower.contains("aspnet") && (body_lower.contains("exception") || body_lower.contains("error")) {
            result.add_finding(
                Finding::new(FindingType::Misconfiguration, "ASP.NET Detailed Errors")
                    ...
                    .with_severity(FindingSeverity::Medium)
                    .with_remediation("Set customErrors mode=\"RemoteOnly\" or \"On\" in web.config"),
            );
        }
        ...
        if body_lower.contains("django") && body_lower.contains("debug") && body_lower.contains("true") {
            result.add_finding(
        ...
        if body_lower.contains("git") && body_lower.contains("ref:") {
            result.add_finding(
                Finding::new(FindingType::Vulnerability, "Git Repository Exposed")
                    .with_description(".git directory is accessible, potentially exposing source code")
        ...
        for indicator in backup_indicators {
            if headers_lower.contains(&format!("content-disposition: attachment; filename=\"{}", indicator))
        ...
        result.add_output(&format!("HTTP vulnerability scan complete for {}:{}", ctx.host, ctx.port));
        Ok(result)
    }
}
    ...
    fn check_server_vulns(&self, server: &str, result: &mut ScriptResult) {
        ...
        if server.contains("apache") {
            if server.contains("2.4.49") || server.contains("2.4.50") {
                result.add_finding(
                    Finding::new(FindingType::Vulnerability, "Apache Path Traversal (CVE-2021-41773)")
                ...
                if self.version_lt(&version, "1.20.0") {
                    result.add_finding(
                        Finding::new(FindingType::Vulnerability, "Outdated Nginx Version")
                            .with_description(&format!("Nginx {} may have known vulnerabilities", version))
                            .with_severity(FindingSeverity::Medium)
...
mod tests {
    ...
    fn test_directory_listing() {
        ...
        let has_dir_listing = result.findings.iter().any(|f| f.title.contains("Directory Listing"));
    ...
    fn test_apache_cve() {
        ...
        let result = script.run(&ctx).unwrap();
        let has_cve = result.findings.iter().any(|f| f.cve == Some("CVE-2021-41773".to_string()));
        assert!(has_cve);
    }
}
