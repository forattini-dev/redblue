...
impl Script for PostgresInfoScript {
  ...
  fn run(&self, ctx: &ScriptContext) -> Result<ScriptResult, String> {
    ...
    match auth_lower.as_str() {
      "trust" => {
        result.add_finding(
          ...
          .with_remediation("Change pg_hba.conf to use 'scram-sha-256' or 'md5' authentication"),
        );
      }
      "md5" => {
        result.add_finding(
    ...
    // Check for SSL/TLS
    if data_lower.contains("ssl") && (data_lower.contains("off") || data_lower.contains("disabled"))
    {
      result.add_finding(
