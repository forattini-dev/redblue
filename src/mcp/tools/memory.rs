...
fn tool_memory_scan(_server: &mut McpServer, args: &JsonValue) -> Result<ToolResult, String> {
  ...
  let regions =
    parse_maps(pid).map_err(|e| format!("Failed to read memory maps for PID {}: {}", pid, e))?;
...
fn tool_memory_regions(_server: &mut McpServer, args: &JsonValue) -> Result<ToolResult, String> {
  ...
  let regions = parse_maps(pid).map_err(|e| format!("Failed to read /proc/{}/maps: {}", pid, e))?;
...
fn tool_memory_read(_server: &mut McpServer, args: &JsonValue) -> Result<ToolResult, String> {
  ...
  // Attach and read
  let mut proc_mem =
    ProcessMemory::attach(pid).map_err(|e| format!("Failed to attach to PID {}: {}", pid, e))?;
...
fn tool_memory_pattern(_server: &mut McpServer, args: &JsonValue) -> Result<ToolResult, String> {
  ...
  // Parse the pattern
  let pattern =
    Pattern::parse(pattern_str).map_err(|e| format!("Invalid pattern '{}': {}", pattern_str, e))?;
