...
pub fn register_code_tools() -> Vec<ToolDefinition<McpServer>> {
  vec![
    ...
    ToolDefinition {
      ...
      description:
        "Validate a secret (API key, token) against its provider API to check if it's active.",
      fields: &[
        ToolField {
          name: "secret",
          field_type: "string",
...
fn tool_secrets_validate(_server: &mut McpServer, args: &JsonValue) -> Result<ToolResult, String> {
  ...
  let provider_name = match provider {
    Some(p) => p,
    None => {
      detect_provider(secret).ok_or("Could not auto-detect provider. Please specify --provider.")?
    }
  };
