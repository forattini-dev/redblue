...
fn tool_binary_pattern_offset(
  ...
  let search_bytes = if value.starts_with("0x") {
    let hex_val =
      u32::from_str_radix(value.trim_start_matches("0x"), 16).map_err(|_| "Invalid hex value")?;
...
fn tool_binary_pack(_server: &mut McpServer, args: &JsonValue) -> Result<ToolResult, String> {
  ...
  let value = if value_str.starts_with("0x") {
    u64::from_str_radix(value_str.trim_start_matches("0x"), 16).map_err(|_| "Invalid hex value")?
  } else {
    value_str
      .parse::<u64>()
