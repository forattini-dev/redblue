...
impl RuleCondition {
  ...
  pub fn matches(&self, event: &Event, context: &CorrelationContext) -> bool {
    match self {
      ...
      RuleCondition::EntityType(et) => event.entity.as_ref().map(|e| &e.entity_type) == Some(et),
      ...
      RuleCondition::CountThreshold {
        ...
        count,
        window_secs,
      } => context.event_count_in_window(*event_type, Duration::from_secs(*window_secs)) >= *count,
    ...
    for (rule_name, actions) in triggered {
      for action in actions {
        ...
        self
          ...
          .write()
          .unwrap()
          .push((rule_name.clone(), action.clone(), event.id));
