...
fn aes256_key_expansion(key: &[u8; 32]) -> Vec<u8> {
  ...
  for i in 8..60 {
    ...
    if i % 8 == 0 {
      ...
      for j in 0..4 {
        expanded[word_offset + j] = expanded[word_offset - 4 + j] ^ expanded[word_offset - 32 + j];
      }
    }
  }
...
mod tests {
  ...
  fn test_aes256_gcm_known_vector_incremental() {
    ...
    let expected_ciphertext = [
      0x47, 0x03, 0xd4, 0x18, 0xc1, 0xe0, 0xc4, 0x1c, 0x85, 0x48, 0x9d, 0x80, 0xbd, 0xe4, 0x76,
      0x62, 0x93, 0xc7, 0x95,
    ];
    let expected_tag = [
      0xb8, 0x5a, 0x13, 0xe8, 0x00, 0x60, 0xf3, 0xd9, 0xd1, 0x7a, 0xda, 0x50, 0xd8, 0x36, 0x58,
      0x60,
    ];
    ...
    let decrypted =
      aes256_gcm_decrypt(&key, &iv, &aad, &ciphertext_tag).expect("decrypt incremental vector");
    assert_eq!(decrypted, plaintext);
  }
