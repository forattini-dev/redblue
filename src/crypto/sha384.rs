...
impl Sha384 {
    ...
    pub fn finalize(mut self) -> [u8; 48] {
        ...
        // Pad with zeros until buffer length is 112 bytes (896 bits)
        while (self.buffer_len % 128) != 112 {
            self.update(&[0x00]);
        }

        // Append message length (128-bit big-endian)
        self.update(&bit_len.to_be_bytes());

        // At this point, buffer should be empty and state finalized
        let mut out = [0u8; 48];
...
/// Convenience function hashing the entire input in one shot.
