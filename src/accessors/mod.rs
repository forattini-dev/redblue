...
impl JsonDecode for AccessorInfo {
  fn from_json_value(value: Value) -> Result<Self, String> {
    ...
    Ok(Self {
      ...
      description: String::from_json_value(map.get("description").cloned().unwrap_or(Value::Null))?,
      methods: Vec::<String>::from_json_value(map.get("methods").cloned().unwrap_or(Value::Null))?,
    })
  }
}
