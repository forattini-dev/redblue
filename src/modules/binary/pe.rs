...
impl PeParser {
    ...
    pub fn parse(path: PathBuf, data: Vec<u8>, format: BinaryFormat) -> BinaryResult<Binary> {
        ...
        let section_headers =
            Self::parse_section_headers(&data, section_offset, number_of_sections)?;
    ...
    fn parse_imports(
        ...
        // RVA to file offset
        let Some(file_offset) = Self::rva_to_file_offset(import_dir.virtual_address, sections)
        else {
            return Ok(imports);
        };
                ...
                if let Some(int_offset) = Self::rva_to_file_offset(original_first_thunk, sections) {
                    ...
                    loop {
                        ...
                        if hint_name_rva & 0x80000000 == 0 {
                            if let Some(hint_offset) =
                                Self::rva_to_file_offset(hint_name_rva, sections)
                            {
