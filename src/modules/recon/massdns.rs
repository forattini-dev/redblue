...
impl MassDnsScanner {
  ...
  pub fn bruteforce(&self, domain: &str, wordlist: &[String]) -> Result<MassDnsResult, String> {
    ...
    let work_queue: Arc<Mutex<Vec<String>>> =
      Arc::new(Mutex::new(wordlist.iter().cloned().collect()));
    ...
    for thread_id in 0..self.config.threads {
      ...
      let handle = thread::spawn(move || {
        let client = DnsClient::new(&resolver).with_timeout(config.timeout.as_millis() as u64);
  ...
  fn detect_wildcard(&self, domain: &str) -> (bool, Vec<String>) {
    ...
    let client = DnsClient::new(resolver).with_timeout(self.config.timeout.as_millis() as u64);
