  ...
  fn check_platform(
    ...
    match response {
      Ok(resp) => {
        ...
        if exists {
          let mut result =
            ProfileResult::found(platform.name, platform.category, &url).with_duration(duration);
          ...
          ProfileResult::not_found(platform.name, platform.category).with_duration(duration)
        }
      }
      Err(e) => ProfileResult::error(platform.name, platform.category, &e).with_duration(duration),
    }
  }
