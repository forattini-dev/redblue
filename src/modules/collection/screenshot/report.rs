...
"#);
    ...
    fn build_card(&self, result: &ScreenshotResult) -> String {
        ...
        let image_html = if let Some(ref thumb_path) = result.thumbnail_path {
            let filename = thumb_path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("screenshot.jpg");
            format!(r#"<img src="{}" alt="Screenshot" loading="lazy">"#, filename)
        ...
        // Technologies badges
        let tech_html: String = result.technologies.iter()
        ...
        let error_html = if let Some(ref error) = result.error {
            format!(r#"<div class="card-meta status-error" style="margin-top: 0.5rem">{}</div>"#,
                    truncate(error, 50))
        } else {
            String::new()
        };
...
"#,
    ...
    pub fn generate_json(&self, results: &BatchResult) -> Result<PathBuf, String> {
        ...
        for (i, result) in results.results.iter().enumerate() {
            ...
            json.push_str(&format!("      \"status_code\": {},\n",
                result.status_code.map(|c| c.to_string()).unwrap_or_else(|| "null".to_string())
            ));
            json.push_str(&format!("      \"load_time_ms\": {},\n", result.load_time_ms));
            json.push_str(&format!("      \"success\": {},\n", result.success()));
            json.push_str(&format!("      \"screenshot_path\": {},\n",
                result.screenshot_path.as_ref()
                    .and_then(|p| p.to_str())
    ...
    pub fn generate_csv(&self, results: &BatchResult) -> Result<PathBuf, String> {
        ...
        let mut csv = String::from("url,title,status_code,load_time_ms,success,screenshot_path,error\n");
