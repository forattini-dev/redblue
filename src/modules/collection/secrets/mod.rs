...
impl SecretScanner {
  ...
  pub fn scan_file(&self, path: &Path) -> Result<Vec<SecretFinding>, String> {
    let file =
      fs::File::open(path).map_err(|e| format!("Failed to open file {}: {}", path.display(), e))?;
  ...
  fn classify_high_risk_file(path: &Path) -> Option<HighRiskFile> {
    ...
    match extension.as_deref() {
      Some("zip") | Some("rar") | Some("7z") | Some("gz") => Some(HighRiskFile {
        rule_id: "archive-backup",
        description: "Compressed archive detected. Ensure sensitive exports are stored securely."
          .to_string(),
        secret_label: file_name,
        skip_text_scan: true,
      }),
      Some("sqlite") | Some("db") | Some("rdb") | Some("realm") | Some("mdb") => {
        Some(HighRiskFile {
          ...
          description: "Database file detected. These often contain credentials or PII."
            .to_string(),
          secret_label: file_name,
          skip_text_scan: true,
        })
      ...
      Some("bak") | Some("backup") => Some(HighRiskFile {
        ...
        description: "Backup file detected (.bak). Review contents for embedded secrets."
          .to_string(),
        secret_label: file_name,
      ...
      Some("env") if file_name.contains("backup") || file_name.contains("bak") => {
        Some(HighRiskFile {
          rule_id: "config-backup",
          description: "Environment file backup detected. Ensure secret copies are removed."
            .to_string(),
          secret_label: file_name,
          skip_text_scan: false,
        })
      ...
      _ => {
        if file_name.contains(".env.")
          && (file_name.contains("bak") || file_name.contains("backup"))
        {
          return Some(HighRiskFile {
            rule_id: "config-backup",
            description: "Environment file backup detected. Remove stale copies of secrets."
              .to_string(),
            secret_label: file_name,
            skip_text_scan: false,
          });
        ...
        if file_name.ends_with(".env.enc") {
          return Some(HighRiskFile {
            ...
            description: "Encrypted environment file detected (.env.enc). Protect decryption keys."
              .to_string(),
            secret_label: file_name,
            skip_text_scan: false,
          });
