...
impl Verifier {
    ...
    pub fn verify_google_api_key(key: &str) -> bool {
        ...
        let url = format!("https://www.googleapis.com/webfonts/v1/webfonts?key={}", key);
        ...
        match client.send(&request) {
            Ok(response) => {
                ...
                response.status_code == 200 || response.status_code == 400 || response.status_code == 403
            }
            Err(_) => false,
        }
    }
    ...
    pub fn verify_stripe_api_key(key: &str) -> bool {
        ...
        request.headers.insert("Authorization".to_string(), auth_header_value);
        ...
        match client.send(&request) {
            Ok(response) => {
                ...
                response.status_code == 200 || response.status_code == 401 || response.status_code == 403
            }
            Err(_) => false,
        }
    }
    ...
    pub fn verify_github_token(key: &str) -> bool {
        ...
        request.headers.insert("Accept".to_string(), "application/vnd.github.v3+json".to_string());
        request.headers.insert("User-Agent".to_string(), "redblue-secrets-verifier".to_string());
