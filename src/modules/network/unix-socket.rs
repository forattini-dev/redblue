...
impl UnixSocketManager {
  ...
  fn listen(&self, path: &Path) -> Result<(), String> {
    ...
    if path.exists() {
      std::fs::remove_file(path).map_err(|e| format!("Failed to remove existing socket: {}", e))?;
    }
  ...
  fn connect(&self, path: &Path) -> Result<(), String> {
    ...
    // Connect to socket
    let mut stream =
      UnixStream::connect(path).map_err(|e| format!("Failed to connect to Unix socket: {}", e))?;
