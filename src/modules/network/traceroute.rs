...
impl Traceroute {
  ...
  fn resolve_target(&self) -> Result<IpAddr, String> {
    ...
    let cfg = config::get();
    let dns = DnsClient::new(&cfg.network.dns_resolver).with_timeout(cfg.network.dns_timeout_ms);
    let answers = dns
    ...
    loop {
      ...
      match icmp_type {
        0 => {
          ...
          let resp_id = u16::from_be_bytes([buffer[ip_header_len + 4], buffer[ip_header_len + 5]]);
          let resp_seq = u16::from_be_bytes([buffer[ip_header_len + 6], buffer[ip_header_len + 7]]);
        ...
        11 | 3 => {
          ...
          let inner_id =
            u16::from_be_bytes([buffer[inner_icmp_offset + 4], buffer[inner_icmp_offset + 5]]);
          let inner_seq =
            u16::from_be_bytes([buffer[inner_icmp_offset + 6], buffer[inner_icmp_offset + 7]]);
