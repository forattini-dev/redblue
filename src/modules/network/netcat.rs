        ...
        match stream.read(&mut buf) {
          ...
          Ok(n) => {
            ...
            while accumulated.len() >= 16 {
              ...
              let decrypted = match cipher_recv_clone.lock().unwrap().decrypt(&to_decrypt) {
                Ok(data) => data,
                Err(e) => {
    ...
    loop {
      ...
      match stdin_reader.read_line(&mut line) {
        ...
        Ok(_) => {
          ...
          let destination = { destination_addr.lock().ok().and_then(|guard| guard.clone()) };

          if let Some(dest) = destination {
            if let Err(e) = server.send_to(&ciphertext, &dest) {
              if self.config.verbose {
                eprintln!("[!] Failed to send encrypted UDP packet to {}: {}", dest, e);
              }
            ...
            eprintln!(
              "[!] No UDP client observed yet; waiting for incoming packet before replying"
            );
          }
