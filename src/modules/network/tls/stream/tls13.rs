...
use super::super::types::{
  ContentType, EcdheParameters, HandshakeType, Tls13NamedGroup, Tls13NewSessionTicket, TlsVersion,
  P256_ORDER_BYTES, TLS13_SERVER_CERT_VERIFY_CONTEXT,
};
...
impl TlsStream {
  ...
  pub(super) fn process_tls13_server_hello(&mut self, record: &[u8]) -> Result<(), String> {
    ...
    let body_len = ((record[1] as usize) << 16) | ((record[2] as usize) << 8) | record[3] as usize;
  ...
  fn parse_tls13_extensions(
    ...
    while ext_offset + 4 <= extensions.len() {
      ...
      match ext_type {
        0x002b => {
          if ext_body.len() != 2 {
            return Err("Invalid supported_versions extension in TLS 1.3 ServerHello".to_string());
          }
          supported_version = Some(u16::from_be_bytes([ext_body[0], ext_body[1]]));
        }
        0x0033 => {
          if ext_body.len() < 4 {
            return Err("Invalid key_share extension in TLS 1.3 ServerHello".to_string());
          }

          let mut body = ext_body;
          if ext_body.len() >= 2
            && (ext_body.len() - 2) == u16::from_be_bytes([ext_body[0], ext_body[1]]) as usize
          {
            body = &ext_body[2..];
          }

          if body.len() < 4 {
            return Err("Invalid key_share extension body in TLS 1.3 ServerHello".to_string());
          }
    ...
    let version = supported_version
      .ok_or_else(|| "TLS 1.3 ServerHello missing supported_versions extension".to_string())?;
    ...
    while offset + 4 <= payload.len() {
      ...
      match handshake_type {
        ...
        HandshakeType::Finished => {
          ...
          let expected = {
            let schedule = self
              .tls13_key_schedule
              .as_ref()
              .ok_or_else(|| "TLS 1.3 key schedule missing for server Finished".to_string())?;
            schedule.server_finished_verify_data()?
          };

          if verify_data != expected {
            return Err("Server Finished verify data mismatch in TLS 1.3 handshake".to_string());
          }
        ...
        HandshakeType::KeyUpdate => {
          return Err(
            "Server sent TLS 1.3 KeyUpdate; key updates are not implemented yet".to_string(),
          );
        }
        other => {
  ...
  fn verify_tls13_certificate_verify_message(&mut self, message: &[u8]) -> Result<(), String> {
    ...
    let signed_data =
      build_tls13_certificate_verify_input(TLS13_SERVER_CERT_VERIFY_CONTEXT, &transcript_hash);
  ...
  fn verify_tls13_certificate_verify(
    ...
    match signature_scheme {
      ...
      0x0401 => {
        ...
        rsa
          .verify_pkcs1_v15(&digest, signature)
      ...
      0x0804 => {
        ...
        let hash = sha256::sha256(signed_data);
        rsa
          .verify_pss_sha256(&hash, signature)
      ...
      0x0805 => {
        ...
        let hash = sha384::sha384(signed_data);
        rsa
          .verify_pss_sha384(&hash, signature)
          .map_err(|e| format!("RSA-PSS verification failed: {}", e))
      }
      _ => Err(format!(
