...
impl Read for TlsStream {
  ...
  fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
    ...
    if self.buffer_pos < self.read_buffer.len() {
      ...
      buf[..to_copy].copy_from_slice(&self.read_buffer[self.buffer_pos..self.buffer_pos + to_copy]);
    ...
    loop {
      ...
      let data = if self.server_encryption_active
        ...
        self
          .decrypt_record_payload(content_type, &payload)
...
impl Write for TlsStream {
  ...
  fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
    ...
    while offset < buf.len() {
      ...
      self
        .send_record(ContentType::ApplicationData, chunk, true)
