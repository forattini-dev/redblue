...
impl ComprehensiveTlsAuditor {
  ...
  fn calculate_grade(
    ...
    let supports_ssl = scan_results
      .iter()
      .any(|r| r.supported && (r.version.as_str() == "SSLv2" || r.version.as_str() == "SSLv3"));
    if supports_ssl {
      return SecurityGrade::D;
    }

    // Check for TLS 1.0/1.1 support
    let supports_old_tls = scan_results
      .iter()
      .any(|r| r.supported && (r.version.as_str() == "TLS 1.0" || r.version.as_str() == "TLS 1.1"));

    // Check if TLS 1.2 or 1.3 is supported
    let supports_modern_tls = scan_results
      .iter()
      .any(|r| r.supported && (r.version.as_str() == "TLS 1.2" || r.version.as_str() == "TLS 1.3"));
