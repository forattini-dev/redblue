...
impl TaxiiClient {
  ...
  pub fn get_objects(
    ...
    let envelope: Envelope =
      crate::serde_json::from_str(&body).map_err(|e| format!("Failed to parse envelope: {}", e))?;
...
impl JsonDecode for CollectionsResponse {
  fn from_json_value(value: Value) -> Result<Self, String> {
    ...
    let collections =
      Vec::<Collection>::from_json_value(map.get("collections").cloned().unwrap_or(Value::Null))?;
    Ok(Self { collections })
  }
}
...
impl JsonDecode for Envelope {
  fn from_json_value(value: Value) -> Result<Self, String> {
    ...
    Ok(Self {
      ...
      next: Option::<String>::from_json_value(map.get("next").cloned().unwrap_or(Value::Null))?,
