...
pub fn generate_iptables_rules(port: u16, mode: TransparentMode) -> Vec<String> {
  match mode {
    TransparentMode::Redirect => vec![
      ...
      format!(
        "iptables -t nat -A PREROUTING -p tcp ! -d 127.0.0.0/8 -j REDIRECT --to-port {}",
        port
      ),
      format!(""),
      ...
      format!("# To remove:"),
      format!(
        "# iptables -t nat -D PREROUTING -p tcp ! -d 127.0.0.0/8 -j REDIRECT --to-port {}",
        port
      ),
    ],
    TransparentMode::TProxy => vec![
      ...
      format!(
        "# iptables -t mangle -A PREROUTING -p udp -j TPROXY --tproxy-mark 0x1/0x1 --on-port {}",
        port
      ),
      format!(""),
      format!("# To remove:"),
      format!(
        "# iptables -t mangle -D PREROUTING -p tcp -j TPROXY --tproxy-mark 0x1/0x1 --on-port {}",
        port
      ),
...
pub fn generate_nftables_rules(port: u16, mode: TransparentMode) -> Vec<String> {
  match mode {
    TransparentMode::Redirect => vec![
      ...
      format!(
        "nft add rule ip nat prerouting tcp dport {{ 80, 443 }} redirect to :{}",
        port
      ),
      format!(""),
      format!("# To remove:"),
    ...
    TransparentMode::TProxy => vec![
      ...
      format!(
        "nft add rule ip mangle prerouting tcp dport {{ 80, 443 }} tproxy to :{} mark set 1",
        port
      ),
      format!(""),
