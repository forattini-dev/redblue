  ...
  fn enumerate_modules(&self, base_url: &str) -> Result<Vec<DrupalModule>, String> {
    ...
    for module_name in popular_modules {
      for path in &module_paths {
        let module_url = format!("{}/{}/{}/{}.info", base_url, path, module_name, module_name);
  ...
  fn check_vulnerabilities(&self, version: &str) -> Vec<DrupalVulnerability> {
    ...
    if major == 7 {
      vulns.push(DrupalVulnerability {
        ...
        description: "Drupal 7 reaches end of life in 2025. No more security updates after EOL."
          .to_string(),
    ...
    if (major == 7 && minor < 59) || (major == 8 && minor < 5) {
      vulns.push(DrupalVulnerability {
        ...
        severity: VulnSeverity::Critical,
        description:
          "RCE vulnerability related to Drupalgeddon 2, allows authenticated users to execute code."
            .to_string(),
