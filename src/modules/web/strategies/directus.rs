...
impl DirectusScanner {
      ...
      if response.status_code == 200 {
        ...
        if let Some(start) = body_str.find("\"version\":\"") {
          ...
          if let Some(end) = body_str[version_start..].find('"') {
            result.version = Some(body_str[version_start..version_start + end].to_string());
          }
        }
  ...
  fn check_admin_panel(&self, url: &str, vulnerabilities: &mut Vec<DirectusVulnerability>) -> bool {
  ...
  fn check_file_upload(&self, url: &str, vulnerabilities: &mut Vec<DirectusVulnerability>) -> bool {
    ...
    for payload in test_payloads {
      ...
      if let Ok(response) = self.http_client.get(&test_url) {
        if response.status_code == 500 {
          ...
          if body_str.contains("SQL") || body_str.contains("syntax") || body_str.contains("mysql") {
            vulnerabilities.push(DirectusVulnerability {
                            severity: VulnSeverity::High,
    ...
    if let Ok(response) = self.http_client.get(&directus_dir) {
      if response.status_code == 200 {
        vulnerabilities.push(DirectusVulnerability {
          ...
          description:
            "The /.directus directory is accessible, potentially exposing configuration files."
              .to_string(),
          evidence: Some(format!("GET {} returned 200 OK", directus_dir)),
          remediation: "Restrict access to /.directus directory using web server configuration."
            .to_string(),
        });
      }
    }
