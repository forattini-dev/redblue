    ...
    for component in popular_components {
      ...
      if let Ok(response) = self.client.get(&component_url) {
        if (200..400).contains(&response.status_code) && !found.contains(component) {
          // Try to get manifest
          let manifest_url = format!("{}/components/{}/{}.xml", base_url, component, component);
  ...
  fn check_vulnerabilities(&self, version: &str) -> Vec<JoomlaVulnerability> {
    ...
    if major == 3 {
      vulns.push(JoomlaVulnerability {
        ...
        description: "Joomla 3.x reached end of life in August 2023. No more security updates."
          .to_string(),
    ...
    if major == 3 && minor < 8 {
      vulns.push(JoomlaVulnerability {
        ...
        description: "Cross-site scripting vulnerability in custom fields component.".to_string(),
