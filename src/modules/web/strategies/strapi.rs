...
impl StrapiScanner {
  ...
  fn check_env_exposure(&self, url: &str, vulnerabilities: &mut Vec<StrapiVulnerability>) -> bool {
  ...
  fn check_vulnerable_endpoints(&self, url: &str, vulnerabilities: &mut Vec<StrapiVulnerability>) {
    ...
    if let Ok(response) = self.http_client.get(&health_url) {
      if response.status_code == 200 {
        vulnerabilities.push(StrapiVulnerability {
          ...
          description:
            "The /_health endpoint is publicly accessible, revealing system status information."
              .to_string(),
          evidence: Some(format!("GET {} returned 200 OK", health_url)),
          remediation:
            "Consider restricting access to health check endpoints to monitoring systems only."
              .to_string(),
        });
      }
    }
