      ...
      if response.status_code == 200 {
        ...
        if body.contains("Django administration") || body.contains("csrfmiddlewaretoken") {
          ...
          fp.admin_login_finding = Some(DjangoFinding {
            ...
            description:
              "The Django admin login page is exposed on the default /admin/login/ path."
                .to_string(),
            evidence: Some(admin_url.clone()),
            remediation: "Restrict admin access via IP allow lists, VPN, or custom admin URLs."
              .to_string(),
          });
        }
      }
    ...
    if !fp.detected {
      if let Ok(response) = self.http_client.get(base_url) {
        ...
        for (key, value) in lower_headers {
          if key == "set-cookie" && (value.contains("csrftoken") || value.contains("sessionid")) {
            fp.detected = true;
          }
        }
      }
