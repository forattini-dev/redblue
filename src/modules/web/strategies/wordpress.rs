...
impl WPScanner {
  ...
  fn check_vulnerabilities(&self, base_url: &str, version: &Option<String>) -> Vec<Vulnerability> {
    ...
    if let Ok(response) = self.client.get(&xmlrpc_url) {
      if response.status_code == 200 || response.status_code == 405 {
        vulns.push(Vulnerability {
          ...
          description: "XML-RPC is enabled and can be abused for brute force attacks".to_string(),
      ...
      if response.status_code == 200 {
        ...
        if body.contains("WP_DEBUG") || body.contains("Fatal error") {
          vulns.push(Vulnerability {
            ...
            description: "WordPress debug mode may expose sensitive information".to_string(),
            path: None,
          });
        }
      }
