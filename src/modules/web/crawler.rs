...
impl WebCrawler {
  ...
  fn fetch_page(&self, url: &str, _base_url: &str) -> Result<CrawledPage, String> {
    ...
    if let Some(ref recorder) = self.har_recorder {
      use crate::protocols::har::{
        iso8601_now, parse_query_string, HarCache, HarContent, HarEntry, HarHeader, HarRequest,
  ...
  fn extract_assets(&self, html: &str, base_url: &str) -> Vec<Asset> {
    ...
    assets.extend(self.extract_tag_attr(html, "link", "href", base_url, AssetType::CSS, &mut seen));

    // Images: <img src="...">
    assets.extend(self.extract_tag_attr(html, "img", "src", base_url, AssetType::Image, &mut seen));
