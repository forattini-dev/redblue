    ...
    if let Some(query_start) = url.find('?') {
      ...
      for pair in query.split('&') {
        if let Some(eq_pos) = pair.find('=') {
          ...
          let result =
            self.scan_parameter(url, name, value, InjectionPoint::GetParam, param_request);
  ...
  fn indicates_successful_auth(failed: &HttpResponse, test: &HttpResponse) -> bool {
    ...
    if test.status == 302 || test.status == 301 {
      if let Some(location) = test.headers.get("location") {
        if location.contains("dashboard") || location.contains("admin") || location.contains("home")
        {
          return true;
        }
      }
...
mod tests {
  ...
  #[test]
  fn test_scan_result() {
    let result = ScanResult::not_vulnerable("http://test.com", "user", InjectionPoint::GetParam);
