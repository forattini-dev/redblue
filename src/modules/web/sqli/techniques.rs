...
impl BooleanBlindDetector {
    ...
    F: Fn(&str) -> HttpResponse,
  {
    ...
    let true_similarity = Self::response_similarity(&baseline_response.body, &true_response.body);
    let false_similarity = Self::response_similarity(&baseline_response.body, &false_response.body);
      ...
      && false_response.status_code != baseline_response.status_code
    {
      ...
      result.evidence.push(format!(
        ...
        baseline_response.status_code, true_response.status_code, false_response.status_code
      ));
...
impl TimeBlindDetector {
    ...
    F: Fn(&str) -> HttpResponse,
  {
    ...
    let threshold_ms = baseline_time.as_millis() as u64 + (config.time_threshold * 1000.0) as u64;
