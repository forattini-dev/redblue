    ...
    if let Some(response) = self.fetch(&changelog_url) {
      if response.status_code == 200 && response.contains("Drupal") {
        findings.push(Finding {
          ...
          description: "Drupal CHANGELOG.txt is publicly accessible, exposing version information"
            .to_string(),
      ...
      if response.status_code == 200 || response.status_code == 403 {
        ...
        if response.contains("Index of") {
          findings.push(Finding {
            ...
            description: "Private files directory is accessible with directory listing enabled"
              .to_string(),
