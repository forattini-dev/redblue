...
pub fn analyze_dns_version(version_response: &str) -> BannerInfo {
  ...
  // dnsmasq detection
    ...
    if let Some(version) = extract_version_pattern(version_response, "dnsmasq-", &[" ", "\r", "\n"])
    {
      info.version = Some(version);
  ...
  // PowerDNS detection
    ...
    if let Some(version) = extract_version_pattern(version_response, "Server", &[" ", "\r", "\n"]) {
      info.version = Some(version);
    }
  ...
  // Unbound detection
    ...
    if let Some(version) = extract_version_pattern(version_response, "unbound", &[" ", "\r", "\n"])
    {
      info.version = Some(version);
    }
  ...
  // Knot DNS detection
    ...
    if let Some(version) = extract_version_pattern(version_response, "Knot DNS", &[" ", "\r", "\n"])
    {
      info.version = Some(version);
    }
