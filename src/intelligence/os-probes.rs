...
use crate::protocols::raw::{
  get_source_ip, internet_checksum, Ipv4Header, TcpHeader, IPPROTO_TCP, TCP_ACK, TCP_FIN, TCP_PSH,
  TCP_SYN, TCP_URG,
};
...
impl OsProber {
  ...
  pub fn new(target: Ipv4Addr) -> Result<Self, String> {
    let src_ip = get_source_ip(target).map_err(|e| format!("Failed to get source IP: {}", e))?;
          ...
          if i + 1 < bytes.len() {
            ...
            if i + len <= bytes.len() {
              ...
              while j + 8 <= i + len {
                ...
                let right =
                  u32::from_be_bytes([bytes[j + 4], bytes[j + 5], bytes[j + 6], bytes[j + 7]]);
...
pub fn quick_os_detect(
  ...
  for sig in db.signatures() {
    ...
    match &sig.window_size {
      ...
      WindowMatch::Multiple(divisor) if *divisor > 0 && window_size.is_multiple_of(*divisor) => {
        confidence += 0.15;
