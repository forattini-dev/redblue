      ...
      match &tokens[i] {
        Token::LongFlag { name, value } => {
          ...
          match long_map.get(name.as_str()) {
            ...
            Some(schema) => {
              self.process_flag(schema, value, tokens, &mut i, &mut flags, &mut errors);
            }
          }
          i += 1;
        }
        ...
        Token::ShortFlag { name, value } => {
          match short_map.get(name) {
            ...
            Some(schema) => {
              ...
              self.process_flag(schema, &str_value, tokens, &mut i, &mut flags, &mut errors);
            }
          }
          i += 1;
        }
...
mod tests {
  use super::*;
  use crate::cli::args::token::tokenize;
  ...
  fn test_parse_positionals_preserved() {
    let parser = make_parser(vec![FlagSchema::boolean("verbose").with_short('v')]);
    let tokens = tokenize(&args(&["scan", "192.168.1.1", "-v", "extra"]));
    let result = parser.parse(&tokens);

    assert!(result.errors.is_empty());
    assert_eq!(result.flags.get("verbose"), Some(&FlagValue::Bool(true)));
    assert_eq!(result.positionals, vec!["scan", "192.168.1.1", "extra"]);
  }

  #[test]
  fn test_parse_mixed_realistic() {
    // rb network ports scan 192.168.1.1 --preset common --threads 500 -vo json --no-color
    let parser = make_parser(vec![
      FlagSchema::new("preset").with_short('p'),
      {
        let mut s = FlagSchema::new("threads").with_short('t');
        s.value_type = ValueType::Integer;
        s
      },
      FlagSchema::boolean("verbose").with_short('v'),
    ...
    assert_eq!(
      result.positionals,
      vec!["network", "ports", "scan", "192.168.1.1"]
    );
    assert_eq!(
      result.flags.get("preset"),
      Some(&FlagValue::Str("common".to_string()))
    );
    assert_eq!(result.flags.get("threads"), Some(&FlagValue::Int(500)));
    assert_eq!(result.flags.get("verbose"), Some(&FlagValue::Bool(true)));
    assert_eq!(
      result.flags.get("output"),
      Some(&FlagValue::Str("json".to_string()))
