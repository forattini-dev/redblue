...
impl MitmCommand {
  ...
  fn start_shell(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    let shell = MitmShell::new(addr).map_err(|e| format!("Failed to initialize shell: {}", e))?;
  ...
  fn export_ca(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    let output_path = match export_format.as_str() {
      "pem" => {
        ...
        std::fs::write(&output_path, &cert_pem).map_err(|e| format!("Failed to write: {}", e))?;
        output_path
      }
      "der" => {
        let output_path = ctx.get_flag_or("output", "mitm-ca-export.der");
        let der_bytes = cert.to_der();
        std::fs::write(&output_path, &der_bytes).map_err(|e| format!("Failed to write: {}", e))?;
        output_path
      }
      _ => {
  ...
  fn configure_hook_mode(
    ...
    if let Some(path) = hook_path {
      let callback = hook_callback
        .ok_or("--hook-callback is required when using --hook-path (e.g., http://10.0.0.1:3000)")?;
