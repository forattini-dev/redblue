...
pub fn whois(ctx: &CliContext) -> Result<(), String> {
  ...
  let payload = build_whois_payload(domain, &result);
  if render::render_machine_output_with_yaml(ctx, "rb recon domain whois", &payload, || {
...
fn build_whois_payload(domain: &str, result: &crate::protocols::whois::WhoisResult) -> Value {
  let mut fields = vec![
