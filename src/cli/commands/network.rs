...
impl NetworkCommand {
  fn ping(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    if render::render_machine_output(ctx, "rb network host ping", &payload)? {
      return Ok(());
    }
  ...
  fn discover(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    let (network, mask) = cidr.split_once('/').unwrap();
    let mask_bits = mask
      .parse::<u8>()
    ...
    if render::render_machine_output(ctx, "rb network host discover", &payload)? {
      return Ok(());
    }

    debug_assert!(is_human);
  ...
  fn fingerprint(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    if ctx.flags.contains_key("persist") {
      ...
      let mut manager =
        StorageService::global().persistence_for_target_with(host, Some(true), None, attributes)?;
  ...
  fn list(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    let db_path = if let Some(ip) = target_ip {
      ...
      return Err("Missing database file. Use --db <file> when listing all hosts.".to_string());
    };
