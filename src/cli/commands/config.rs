...
impl Command for ConfigDatabaseCommand {
  ...
  fn route_metadata(&self, verb: &str) -> crate::cli::schema::RouteMetadata {
    let machine_output = match verb {
      "show" => crate::cli::schema::MachineOutputMetadata::new()
...
impl ConfigDatabaseCommand {
  fn set_password(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    // Save to keyring
    save_to_keyring(&password).map_err(|e| format!("Failed to save password to keyring: {}", e))?;
  ...
  fn show(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    let payload = Self::show_payload(
      config.database.auto_persist,
      config.database.auto_name,
      &db_dir,
      has_keyring,
      has_env,
    );
    if render::render_machine_output(ctx, "rb config database show", &payload)? {
      return Ok(());
    }
    ...
    println!();
    println!("  \x1b[36mTip:\x1b[0m Use --save flag on commands to persist results to database.");
    println!("  \x1b[36mTip:\x1b[0m Password priority: --db-password > REDBLUE_DB_KEY > keyring");
