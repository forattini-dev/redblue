...
pub fn cmd_summary(ctx: &CliContext, graph: &GraphStore, format: &str) -> Result<(), String> {
  ...
  if format == "json" {
    ...
    render::render_machine_output(ctx, "rb intel graph summary", &payload)?;
  } else {
    ...
    println!(
      ...
      if stats.node_count > 1 {
        stats.edge_count as f64 / (stats.node_count as f64 * (stats.node_count as f64 - 1.0))
      } else {
        0.0
      }
    );
...
pub fn cmd_insights(ctx: &CliContext, graph: &GraphStore, format: &str) -> Result<(), String> {
  let stats = graph.stats();
  ...
  if format == "json" {
    ...
    render::render_machine_output(ctx, "rb intel graph insights", &payload)?;
  } else {
...
pub fn cmd_export(ctx: &CliContext, graph: &GraphStore) -> Result<(), String> {
  ...
  let write_result = std::fs::write(&output_path, &content);
  let payload = graph_export_payload(
    format,
    &output_path,
    nodes.len(),
    edges.len(),
    write_result.as_ref().err().map(ToString::to_string),
    match &write_result {
      Ok(_) => None,
      Err(_) => Some(content.as_str()),
    },
  );
  if render::render_machine_output(ctx, "rb intel graph export", &payload)? {
    return Ok(());
  }
  ...
  match write_result {
    ...
    Err(e) => {
      // Print to stdout if file write fails in human mode.
      println!("{}", content);
...
pub fn cmd_report(ctx: &CliContext, graph: &GraphStore) -> Result<(), String> {
  ...
  if let Some((top_node, top_score)) = top_pr.first() {
    if **top_score > 0.1 {
      report.push_str(&format!(
        "- ⚠️ **High Influence**: Node `{}` has disproportionate influence. Consider redundancy.\n",
        top_node
      ));
    }
  }
  ...
  let write_result = std::fs::write(&output_path, &report);
  let payload = graph_report_payload(
    &output_path,
    node_count,
    edge_count,
    components.count,
    ...
    match &write_result {
      Ok(_) => None,
      Err(_) => Some(report.as_str()),
    },
  );
  if render::render_machine_output(ctx, "rb intel graph report", &payload)? {
    return Ok(());
  }
...
pub fn cmd_query(ctx: &CliContext, graph: &GraphStore, format: &str) -> Result<(), String> {
  ...
  let result =
    UnifiedExecutor::execute_on(graph, &query).map_err(|e| format!("Execution error: {:?}", e))?;

  if format == "json" {
    let payload = query_result_payload(&query_str, &result);
    render::render_machine_output(ctx, "rb intel graph query", &payload)?;
  } else {
...
pub fn cmd_rql_help() -> Result<(), String> {
  ...
  println!("│                                                                           │");
  println!(
    "│  \x1b[1mNode Types:\x1b[0m Host, Service, Credential, User, Vulnerability,             │"
  );
  println!("│             Domain, Endpoint, Technology, Certificate, Network            │");
  println!("│                                                                           │");
  println!(
    "│  \x1b[1mEdge Types:\x1b[0m HasService, ConnectsTo, AuthAccess, UsesTech,                │"
  );
  ...
  println!("│                                                                           │");
  println!(
    "│  \x1b[1mOperators:\x1b[0m =, !=, <, >, <=, >=, LIKE, IN, BETWEEN, AND, OR, NOT         │"
  );
  println!("│                                                                           │");
  println!(
    "│  \x1b[1mExamples:\x1b[0m                                                                 │"
  ...
  println!(
    "\x1b[1;36m┌─── NATURAL LANGUAGE MODE ────────────────────────────────────────────────┐\x1b[0m"
  );
  ...
  // Usage Examples
  println!(
    "\x1b[1;33m┌─── QUICK EXAMPLES ──────────────────────────────────────────────────────┐\x1b[0m"
  );
...
/// Graph stats command
pub fn cmd_stats(ctx: &CliContext, graph: &GraphStore, format: &str) -> Result<(), String> {
  let nodes: Vec<_> = graph.iter_nodes().collect();
  let edges = graph.iter_all_edges();
  ...
  if format == "json" {
    let payload = graph_stats_payload(
      nodes.len(),
      edges.len(),
      density,
      avg_degree,
    ...
    render::render_machine_output(ctx, "rb intel graph stats", &payload)?;
  } else {
