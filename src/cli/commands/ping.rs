...
use crate::cli::{format::OutputFormat, output::Output, render};
use crate::json;
...
impl PingCommand {
  fn ping(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    if format == OutputFormat::Yaml {
      return Err(
        "YAML output is not supported for `rb network host ping` yet. Use `--json` or human output."
          .to_string(),
      );
    }
    ...
    let payload = Self::ping_payload(host, count, interval, timeout, packet_size, &result);
