...
impl FileCommand {
  ...
  fn compress(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    match format.as_str() {
      ...
      "tar" => Err("TAR creation not yet implemented. Only reading is available.".to_string()),
      "zip" => Err("ZIP creation not yet implemented. Only reading is available.".to_string()),
      _ => Err(format!(
  ...
  fn decompress_gzip(
    ...
    fs::write(&out_path, &decompressed).map_err(|e| format!("Failed to write output: {}", e))?;
