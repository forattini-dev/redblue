...
impl CryptoCodecCommand {
  ...
  fn encode(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    let input = if let Some(file) = ctx.get_flag("file") {
      ...
      return Err("No input provided. Usage: rb crypto codec encode <codec> <input>".to_string());
    };
  ...
  fn decode(&self, ctx: &CliContext) -> Result<(), String> {
    ...
    let input = if let Some(file) = ctx.get_flag("file") {
      ...
      return Err("No input provided. Usage: rb crypto codec decode <codec> <input>".to_string());
    };
