...
impl CryptoAnalyzeCommand {
    ...
    fn entropy_analysis(&self, ctx: &CliContext) -> Result<(), String> {
        ...
        let input_bytes = if let Some(file_path) = ctx.get_flag("file") {
            ...
            return Err(
                "No input provided. Use: rb crypto analyze entropy <data> [--file FILE]".into(),
            );
        };
        ...
        if format == "json" {
            ...
            match result.classification {
                crate::crypto::analysis::EntropyClassification::VeryLow => {
                    println!(
                        "  Data is highly uniform/repetitive. Likely sparse or constant data."
                    );
                }
                ...
                crate::crypto::analysis::EntropyClassification::Maximum => {
                    println!(
                        "  Near-maximum entropy indicates encrypted or cryptographically random data."
                    );
                }
            }
        }
