...
impl Asn1Object {
  ...
  pub fn as_oid(&self) -> Result<String, String> {
    match &self.value {
      Asn1Value::ObjectIdentifier(components) => Ok(
        components
          .iter()
          .map(|n| n.to_string())
          .collect::<Vec<_>>()
...
fn parse_value(tag_byte: u8, constructed: bool, data: &[u8]) -> Result<Asn1Value, String> {
  ...
  match tag_number {
    ...
    0x0C => {
      // UTF8String
      let s = String::from_utf8(data.to_vec()).map_err(|e| format!("Invalid UTF8String: {}", e))?;
    ...
    0x13 => {
      // PrintableString
      let s =
        String::from_utf8(data.to_vec()).map_err(|e| format!("Invalid PrintableString: {}", e))?;
      Ok(Asn1Value::PrintableString(s))
    }
    0x16 => {
      // IA5String
      let s = String::from_utf8(data.to_vec()).map_err(|e| format!("Invalid IA5String: {}", e))?;
      Ok(Asn1Value::IA5String(s))
    }
    0x17 => {
      // UTCTime (YYMMDDHHMMSSZ)
      let s = String::from_utf8(data.to_vec()).map_err(|e| format!("Invalid UTCTime: {}", e))?;
      Ok(Asn1Value::UTCTime(s))
    }
    0x18 => {
      // GeneralizedTime (YYYYMMDDHHMMSSZ)
      let s =
        String::from_utf8(data.to_vec()).map_err(|e| format!("Invalid GeneralizedTime: {}", e))?;
