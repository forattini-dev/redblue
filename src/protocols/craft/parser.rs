...
impl SignatureParser {
    ...
    fn parse_line(&self, line: &str, sig: &mut PacketSignature) -> Result<(), ParseError> {
        ...
        let eq_pos = line.find('=').ok_or_else(|| {
            ParseError::SyntaxError(self.line, format!("Expected '=' in: {}", line))
        })?;
    ...
    fn parse_random(&self, field: &LayerField, s: &str) -> Result<FieldValue, ParseError> {
        ...
        match field {
            ...
            | LayerField::UdpSport
            | LayerField::UdpDport => {
                let min: u16 = parts[0].parse().map_err(|_| {
                    ParseError::SyntaxError(self.line, "Invalid min value".to_string())
                })?;
