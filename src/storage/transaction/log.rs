  ...
  pub fn from_bytes(data: &[u8]) -> io::Result<(Self, usize)> {
    ...
    let entry = match tag {
      ...
      4 => {
        ...
        let key = data[offset..offset + key_len].to_vec();
        offset += key_len;
        let old_len = u32::from_le_bytes(data[offset..offset + 4].try_into().unwrap()) as usize;
        offset += 4;
        let old_value = data[offset..offset + old_len].to_vec();
        offset += old_len;
        let new_len = u32::from_le_bytes(data[offset..offset + 4].try_into().unwrap()) as usize;
        offset += 4;
