  ...
  pub fn analyze_findings(&self, domain: &str) -> Vec<DomainFinding> {
    ...
    if !inactive.is_empty() {
      findings.push(DomainFinding {
        ...
        description: "Subdomains pointing to non-existent resources may be vulnerable to takeover"
          .to_string(),
      });
    }
    ...
    if !exposed.is_empty() {
      findings.push(DomainFinding {
        ...
        description: "Database and administrative services should not be publicly accessible"
          .to_string(),
      });
    }
