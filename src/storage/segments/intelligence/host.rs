...
impl<'a> HostIntelligence<'a> {
  ...
  fn resolve_hostname(&self, node: &crate::storage::segments::graph::GraphNode) -> Option<String> {
  ...
  fn count_incoming_paths(&self, host_id: &str) -> usize {
    ...
    if let Some(node) = self.graph.get_node(host_id) {
      node
        .in_edges
        .iter()
        .filter(|e| e.edge_type == EdgeType::AuthAccess || e.edge_type == EdgeType::ConnectsTo)
        .count()
    } else {
      0
    }
