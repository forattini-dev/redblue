...
impl LootEntry {
  ...
  pub fn from_bytes(bytes: &[u8]) -> Result<Self, DecodeError> {
    ...
    let category = LootCategory::from_u8(bytes[pos]).ok_or(DecodeError("invalid loot category"))?;
    pos += 1;
    ...
    let confidence = Confidence::from_u8(bytes[pos]).ok_or(DecodeError("invalid confidence"))?;
    pos += 1;
