...
use crate::storage::records::{IocRecord, IocType};

#[derive(Debug, Clone, Copy)]
struct IocSegmentHeader {
    type_count: u32,
    record_count: u32,
    directory_len: u64,
    payload_len: u64,
...
impl IocSegmentHeader {
    const MAGIC: [u8; 4] = *b"IOC1";
    const VERSION: u16 = 1;
    ...
    fn write(&self, buf: &mut Vec<u8>) {
        ...
        buf.extend_from_slice(&self.type_count.to_le_bytes());
    ...
    fn read(bytes: &[u8]) -> Result<Self, DecodeError> {
        ...
        if version != Self::VERSION {
            return Err(DecodeError("unsupported ioc segment version"));
        }
        let type_count = u32::from_le_bytes(bytes[8..12].try_into().unwrap());
...
fn ioc_type_to_key(t: IocType) -> String {
    match t {
        IocType::IPv4 => "ipv4",
        IocType::IPv6 => "ipv6",
...
fn key_to_ioc_type(k: &str) -> Option<IocType> {
    match k {
        "ipv4" => Some(IocType::IPv4),
        "ipv6" => Some(IocType::IPv6),
        "domain" => Some(IocType::Domain),
        "url" => Some(IocType::URL),
        "email" => Some(IocType::Email),
        "md5" => Some(IocType::HashMD5),
        "sha1" => Some(IocType::HashSHA1),
        "sha256" => Some(IocType::HashSHA256),
        "cert" => Some(IocType::Certificate),
        "ja3" => Some(IocType::JA3),
        _ => None,
    }
}
...
impl IocSegment {
    ...
    pub fn push(&mut self, record: IocRecord) {
        let idx = self.records.len();
        let key = ioc_type_to_key(record.ioc_type);
        self.index.entry(key).or_default().push(idx);
        self.records.push(record);
    }

    pub fn get_by_type(&self, ioc_type: IocType) -> Vec<IocRecord> {
        let key = ioc_type_to_key(ioc_type);
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        let header = IocSegmentHeader {
            type_count: self.index.len() as u32,
    ...
    pub fn deserialize(bytes: &[u8]) -> Result<Self, DecodeError> {
        if bytes.len() < IocSegmentHeader::SIZE {
        ...
        let mut index = HashMap::with_capacity(header.type_count as usize);
...
impl IocSegmentView {
    ...
    pub fn get_by_type(&self, ioc_type: IocType) -> Result<Vec<IocRecord>, DecodeError> {
        let key = ioc_type_to_key(ioc_type);
        if let Some(&(offset, len, count)) = self.directory.get(&key) {
