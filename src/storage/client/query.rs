...
impl QueryManager {
  ...
  pub fn open<P: AsRef<Path>>(path: P) -> io::Result<Self> {
    ...
    let db =
      RedDB::open(path_ref).map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
      ...
      Ok(res) => res
        ...
        .filter_map(|item| {
          ...
          Some(TlsScanRecord {
            ...
            negotiated_cipher_strength: crate::storage::records::TlsCipherStrength::Strong,
            certificate_valid,
            versions: Vec::new(),
            ciphers: Vec::new(),
      ...
      Ok(res) => res
        ...
        .filter_map(|item| {
          ...
          let http_version = node
            ...
            .unwrap_or("HTTP/1.1")
            .to_string();
          let status_code = node.get("status").and_then(|v| v.as_integer()).unwrap_or(0) as u16;
