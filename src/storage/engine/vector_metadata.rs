...
impl MetadataValue {
  ...
  pub fn matches_eq(&self, other: &MetadataValue) -> bool {
    match (self, other) {
      ...
      (MetadataValue::Integer(a), MetadataValue::Float(b)) => (*a as f64 - b).abs() < f64::EPSILON,
      (MetadataValue::Float(a), MetadataValue::Integer(b)) => (a - *b as f64).abs() < f64::EPSILON,
      _ => false,
    }
  }
...
mod tests {
  ...
  fn test_filter_contains() {
    ...
    let filter = MetadataFilter::Contains("description".to_string(), "vulnerability".to_string());
    let results = store.filter(&filter);
