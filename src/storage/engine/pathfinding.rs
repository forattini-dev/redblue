    ...
    for i in 1..k {
      ...
      for spur_idx in 0..prev_path.nodes.len() - 1 {
        ...
        for existing_path in &result {
          if existing_path.nodes.len() > spur_idx && existing_path.nodes[..=spur_idx] == root_path {
        ...
        let excluded_nodes: HashSet<String> = root_path[..spur_idx].iter().cloned().collect();

        // Find spur path
        if let Some(spur_path) =
          Self::dijkstra_with_exclusions(graph, spur_node, target, &excluded_edges, &excluded_nodes)
        {
