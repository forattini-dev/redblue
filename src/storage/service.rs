...
impl StorageService {
  ...
  pub fn refresh_target_partition<P: AsRef<Path>>(&self, target: &str, path: P) -> io::Result<()> {
  ...
  pub fn refresh_partition<P: AsRef<Path>>(
    ...
    match Self::inspect_segments(path) {
      Ok(segments) => {
        let mut metadata = PartitionMetadata::new(key.clone(), label, path.to_path_buf(), segments);
...
mod tests {
  ...
  fn test_storage_service_partitions_filtered() {
    ...
    let scan_partitions =
      service.partitions_filtered(|meta| meta.attributes.get("type") == Some(&"scan".to_string()));
