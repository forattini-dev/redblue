  ...
  pub fn from_bytes(data: &[u8]) -> Result<(Self, usize), ValueError> {
    ...
    let value = match data_type {
      ...
      DataType::VectorRef => {
        ...
        let collection = String::from_utf8_lossy(&data[offset..offset + len as usize]).to_string();
        offset += len as usize;
...
mod tests {
  ...
  fn test_value_uuid() {
    let uuid = [
      0x55, 0x04, 0x43, 0x01, 0x8f, 0x3b, 0x4a, 0x12, 0x9c, 0x5d, 0x6e, 0x7f, 0x80, 0x91, 0xa2,
      0xb3,
    ];
