...
impl SortKey {
    ...
    pub fn compare(&self, a: &Value, b: &Value) -> Ordering {
        ...
        match (a, b) {
            ...
            _ => {
                let base_order = compare_values(a, b);
...
/// Get a numeric tag for a value type (for stable cross-type ordering)
...
fn compare_values(a: &Value, b: &Value) -> Ordering {
    match (a, b) {
        ...
        _ => value_type_tag(a).cmp(&value_type_tag(b)),
    }
}
...
mod tests {
    ...
    fn test_compare_cross_type() {
        assert_eq!(
            compare_values(&Value::Integer(10), &Value::Float(10.0)),
            Ordering::Equal
        );
        assert_eq!(
            compare_values(&Value::Integer(9), &Value::Float(10.0)),
            Ordering::Less
        );
    }
