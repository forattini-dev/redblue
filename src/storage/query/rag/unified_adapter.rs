...
impl UnifiedStoreAdapter {
  ...
  pub fn find_by_cross_ref(
    ...
    while let Some((current_id, depth, path)) = frontier.pop() {
      ...
      if let Some((col_name, entity)) = self.store.get_any(current_id) {
        ...
        for (target_id, link_type, target_collection) in self.store.get_refs_from(current_id) {
  ...
  pub fn expand_entity_context(
    ...
    let refs_result = self.find_by_cross_ref(entity_id, RefType::RelatedTo, config.graph_depth)?;
  ...
  fn matches_metadata(&self, entity: &UnifiedEntity, filter: &MetadataQuery) -> bool {
    ...
    for (key, expected) in &filter.conditions {
      ...
      let matches = match (prop_val, expected) {
        ...
        (Some(Value::Integer(i)), QueryCondition::Equals(QueryValue::Int(exp))) => *i == *exp,
        (Some(Value::Float(f)), QueryCondition::Equals(QueryValue::Float(exp))) => *f == *exp,
        (Some(Value::Boolean(b)), QueryCondition::Equals(QueryValue::Bool(exp))) => *b == *exp,
        (Some(Value::Integer(i)), QueryCondition::GreaterThan(QueryValue::Int(n))) => *i > *n,
        (Some(Value::Float(f)), QueryCondition::GreaterThan(QueryValue::Float(n))) => *f > *n,
        ...
        (Some(Value::Text(s)), QueryCondition::Contains(substr)) => s.contains(substr.as_str()),
        _ => false,
      };
      if !matches {
        return false;
