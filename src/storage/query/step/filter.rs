...
impl Predicate {
  ...
  pub fn test(&self, value: &Value) -> bool {
    match self {
      ...
      Predicate::Lt(expected) => compare_json(value, expected) == Some(std::cmp::Ordering::Less),
      ...
      Predicate::Gt(expected) => compare_json(value, expected) == Some(std::cmp::Ordering::Greater),
