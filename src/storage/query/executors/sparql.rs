...
impl SparqlExecutor {
  ...
  fn predicate_alias_matches(&self, predicate: &str, edge_type: GraphEdgeType) -> bool {
    match (predicate, edge_type) {
      ...
      ("hascert" | "has_cert" | "hascertificate" | "has_certificate", GraphEdgeType::HasCert) => {
        true
      }
      ...
      ("relatedto" | "related_to" | "memberof" | "member_of", GraphEdgeType::RelatedTo) => true,
      _ => false,
    }
  }
        ...
        if let Some(value) = binding.get(col) {
          match value {
            ...
            BoundValue::Edge(from, etype, to) => {
              ...
              record.set(
                col,
                Value::Text(format!("{}->{}({})", from, to, format!("{:?}", etype))),
              );
            }
