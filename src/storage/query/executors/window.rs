...
impl WindowExecutor {
  ...
  fn compute_for_partition(partition: &[IndexedBinding], func: &WindowFunc) -> Vec<IndexedBinding> {
    ...
    partition
      ...
      .enumerate()
      .map(|(row_idx, indexed)| {
        let value = Self::compute_value(partition, row_idx, &peer_groups, partition_size, func);
  ...
  fn compute_value(
    ...
    match &func.func_type {
      ...
      WindowFuncType::FirstValue(var) => {
        // Get frame bounds
        let (start, _) =
          Self::get_frame_bounds(row_idx, partition_size, peer_groups, &func.window.frame);
        partition
          .get(start)
...
mod tests {
  ...
  fn test_partitioned_sum() {
    ...
    let func = WindowFunc::new(
      ...
      Var::new("dept_total"),
      WindowDef::partition_by(vec![Var::new("dept")]).with_frame(FrameSpec::entire_partition()),
    );
