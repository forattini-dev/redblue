...
impl InMemoryHybridExecutor {
  ...
  pub fn execute_with_fusion(
    ...
    match fusion {
      FusionStrategy::Rerank { weight } => self.fuse_rerank(structured, vector_result, *weight),
  ...
  fn fuse_rrf(
    ...
    let mut result = UnifiedResult::with_columns(vec!["id".to_string(), "distance".to_string()]);
    for (mut record, score) in scored {
      record
        .values
        .insert("_rrf_score".to_string(), Value::Float(score));
