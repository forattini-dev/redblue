...
pub use join::{
  choose_strategy, execute_join, hash_join, merge_join, nested_loop_join, JoinCondition, JoinStats,
...
impl MultiModeExecutor {
  ...
  pub fn execute_as(&self, query: &str, mode: QueryMode) -> Result<ExecuteResult, ExecutionError> {
    match mode {
      QueryMode::Sql | QueryMode::Cypher | QueryMode::Path => {
        let ast = parse_multi(query).map_err(|e| ExecutionError::new(e.to_string()))?;
        let result = self.unified.execute(&ast)?;
