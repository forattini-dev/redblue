...
impl GremlinExecutor {
  ...
  pub fn execute(&self, query: &str) -> Result<UnifiedResult, ExecutionError> {
    let traversal = GremlinParser::parse(query).map_err(|e| ExecutionError::new(e.to_string()))?;
          ...
          if let Some(id) = id_opt {
            ...
            if let Some((from, to)) = id.split_once("->") {
              for (edge_type, target, weight) in self.graph.outgoing_edges(from) {
                if target == to {
                  traversers.push(Traverser::at_edge(from, edge_type, &target, weight));
            ...
            for node in self.graph.iter_nodes() {
              for (edge_type, target, weight) in self.graph.outgoing_edges(&node.id) {
                stats.edges_scanned += 1;
                traversers.push(Traverser::at_edge(&node.id, edge_type, &target, weight));
              }
            }
          }
          break;
  ...
  fn execute_step(
    ...
    match step {
      ...
      GremlinStep::Where(inner) => {
        for t in traversers {
          let inner_result = self.execute_inner_traversal(vec![t.clone()], inner, stats)?;
      ...
      GremlinStep::Union(branches) => {
        for t in traversers {
          for branch in branches {
            result.extend(self.execute_inner_traversal(vec![t.clone()], branch, stats)?);
          }
        }
      }
      ...
      GremlinStep::Coalesce(branches) => {
        for t in traversers {
          for branch in branches {
            let branch_result = self.execute_inner_traversal(vec![t.clone()], branch, stats)?;
      ...
      GremlinStep::Fold => {
        result = vec![Traverser {
          current: TraverserElement::Value(GremlinValue::Integer(traversers.len() as i64)),
          path: Vec::new(),
          labels: HashMap::new(),
          bulk: 1,
          loops: HashMap::new(),
    ...
    for t in traversers {
      ...
      match &t.current {
        ...
        TraverserElement::Value(v) => match v {
          ...
          GremlinValue::Float(f) => record.set("_value", crate::storage::schema::Value::Float(*f)),
