...
impl JoinCondition {
  ...
  pub fn left_vars(&self) -> Vec<Var> {
    match self {
      ...
      JoinCondition::And(conditions) => conditions.iter().flat_map(|c| c.left_vars()).collect(),
  ...
  pub fn right_vars(&self) -> Vec<Var> {
    match self {
      ...
      JoinCondition::And(conditions) => conditions.iter().flat_map(|c| c.right_vars()).collect(),
      JoinCondition::None => Vec::new(),
    }
  }
}
...
pub fn hash_join(
  ...
  let (build_side, probe_side, build_keys, probe_keys, is_left_build) = if left.len() <= right.len()
  {
  ...
  match (join_type, is_left_build) {
    (JoinType::Left, true) => {
      ...
      let mut all_left_matched: std::collections::HashSet<usize> = std::collections::HashSet::new();
...
fn check_condition(left: &Binding, right: &Binding, condition: &JoinCondition) -> bool {
  match condition {
    JoinCondition::Eq(left_var, right_var) => match (left.get(left_var), right.get(right_var)) {
      (Some(l), Some(r)) => l == r,
      _ => false,
    },
    JoinCondition::And(conditions) => conditions.iter().all(|c| check_condition(left, right, c)),
