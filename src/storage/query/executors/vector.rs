...
impl InMemoryVectorExecutor {
  ...
  pub fn execute(&self, query: &VectorQuery) -> Result<UnifiedResult, ExecutionError> {
    ...
    let search_results: Vec<(u64, f32)> = if let Some(index) = self.indexes.get(&query.collection) {
...
fn evaluate_filter(filter: &MetadataFilter, metadata: &HashMap<String, MetadataValue>) -> bool {
  match filter {
    MetadataFilter::Eq(field, value) => metadata.get(field).map(|v| v == value).unwrap_or(false),
    MetadataFilter::Ne(field, value) => metadata.get(field).map(|v| v != value).unwrap_or(true),
    MetadataFilter::Lt(field, value) => compare_values(metadata.get(field), value, |a, b| a < b),
    MetadataFilter::Lte(field, value) => compare_values(metadata.get(field), value, |a, b| a <= b),
    MetadataFilter::Gt(field, value) => compare_values(metadata.get(field), value, |a, b| a > b),
    MetadataFilter::Gte(field, value) => compare_values(metadata.get(field), value, |a, b| a >= b),
