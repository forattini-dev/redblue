...
mod tests {
  ...
  fn test_similarity_threshold() {
    ...
    let query = SimilarityQuery::new(DenseVector::new(vec![1.0, 0.0, 0.0]), 10).with_threshold(0.5);
  ...
  fn test_hybrid_search_with_filter() {
    ...
    let query =
      SimilarityQuery::new(DenseVector::new(vec![1.0, 0.0, 0.0]), 5).with_filter(filter.clone());
