...
impl FilterRanker {
  ...
  pub fn estimate_selectivity(&self, filter: &FilterExpr) -> f64 {
    let sel = match filter {
      ...
      FilterExpr::Ne { column, value } => 1.0 - self.estimate_equality_selectivity(column, value),
  ...
  pub fn estimate_cost(&self, filter: &FilterExpr) -> f64 {
    let cost = match filter {
      FilterExpr::Eq { value, .. } | FilterExpr::Ne { value, .. } => self.value_compare_cost(value),
      FilterExpr::Lt { value, .. }
      | FilterExpr::Le { value, .. }
      | FilterExpr::Gt { value, .. }
      ...
      FilterExpr::In { values, .. } => self.config.cost_model.in_per_element * values.len() as f64,
      FilterExpr::Like { pattern, .. } => {
        self.config.cost_model.pattern_match
          + self.config.cost_model.string_per_byte * pattern.len() as f64
      }
      FilterExpr::IsNull { .. } | FilterExpr::IsNotNull { .. } => self.config.cost_model.null_check,
