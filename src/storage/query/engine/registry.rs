...
impl InMemoryEngine {
  ...
  /// Execute join
  fn execute_join(&self, left: Box<dyn BindingIterator>, right_op: Op) -> Box<dyn BindingIterator> {
    ...
    let result: Vec<Binding> = left
      ...
      .filter(|binding| {
        !right.iter().any(|candidate| {
          bindings_share_vars(binding, candidate) && bindings_compatible(binding, candidate)
        })
      })
      .collect();
  ...
  fn build_aggregator(agg: &Aggregate) -> Option<Box<dyn Aggregator>> {
    match agg {
      ...
      Aggregate::GroupConcat(_, sep) => Some(Box::new(GroupConcatAggregator::new(sep.clone()))),
    }
  }
