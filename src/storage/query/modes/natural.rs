...
impl NaturalParser {
  ...
  fn normalize(input: &str) -> String {
    ...
    unquoted
      ...
      .chars()
      .map(|c| {
        if c.is_alphanumeric() || c.is_whitespace() || c == '.' || c == ':' || c == '-' || c == '_'
        {
          c
        } else {
          ' '
    ...
    for (i, part) in parts.iter().enumerate() {
      if *part == keyword {
        ...
        if let Some(next) = parts.get(i + 1) {
          // Skip common words
          if ![
            "with", "that", "has", "have", "is", "are", "the", "a", "an", "for", "on", "in",
          ]
          .contains(next)
          {
  ...
  fn extract_filters(text: &str) -> Vec<PropertyFilter> {
    ...
    let services = [
      "ssh", "http", "https", "ftp", "smtp", "mysql", "postgres", "redis", "mongodb", "rdp", "vnc",
    ];
    for svc in services {
  ...
  fn detect_relationship(
    ...
    if text.contains("has access") || text.contains("can access") || text.contains("authenticate") {
    ...
    match (primary, secondary) {
      (Some(EntityType::Host), Some(EntityType::Service)) => Some(RelationshipType::HasService),
      (Some(EntityType::Host), Some(EntityType::Port)) => Some(RelationshipType::HasPort),
      (Some(EntityType::Host), Some(EntityType::Vulnerability)) => Some(RelationshipType::HasVuln),
      (Some(EntityType::User), Some(EntityType::Credential)) => {
        Some(RelationshipType::HasCredential)
      }
      (Some(EntityType::Credential), Some(EntityType::Host)) => Some(RelationshipType::AuthAccess),
      (Some(EntityType::Vulnerability), Some(EntityType::Host)) => Some(RelationshipType::Affects),
      _ => None,
    }
  }
