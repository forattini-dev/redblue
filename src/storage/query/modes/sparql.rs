...
mod tests {
  ...
  fn test_parse_with_prefix() {
    let q =
      SparqlParser::parse("PREFIX ex: <http://example.org/> SELECT ?x WHERE { ?x ex:type ?t }")
        .unwrap();
  ...
  fn test_parse_multiple_patterns() {
    let q =
      SparqlParser::parse("SELECT ?host ?ip WHERE { ?host :hasIP ?ip . ?host :hasName ?name }")
        .unwrap();
  ...
  fn test_parse_with_filter() {
    let q =
      SparqlParser::parse("SELECT ?host WHERE { ?host :port ?p } FILTER (?p > 1000)").unwrap();
