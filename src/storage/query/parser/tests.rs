...
fn test_parse_join_query() {
  let query = parse("FROM hosts h JOIN GRAPH (n:Host)-[:AFFECTED_BY]->(v) ON h.ip = n.id").unwrap();
...
fn test_parse_hybrid_query() {
  ...
  if let QueryExpr::Hybrid(hq) = query {
    ...
    assert!(matches!(hq.fusion, FusionStrategy::Rerank { weight } if (weight - 0.7).abs() < 0.01));
...
fn test_parse_fusion_strategies() {
  ...
  // RRF
  let query =
    parse("HYBRID FROM hosts VECTOR SEARCH e SIMILAR TO [0.1] FUSION RRF(30) LIMIT 10").unwrap();
  if let QueryExpr::Hybrid(hq) = query {
