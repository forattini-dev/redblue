use super::super::entity::UnifiedEntity;
...
fn test_store_auto_create() {
  ...
  let entity = UnifiedEntity::vector(store.next_entity_id(), "embeddings", vec![0.1, 0.2, 0.3]);
...
fn test_cross_references() {
  ...
  store
    .add_cross_ref(
      ...
      "vulns",
      vuln_id,
      super::super::entity::RefType::RelatedTo,
      1.0,
    )
    .unwrap();
...
fn test_expand_refs() {
  ...
  store
    .add_cross_ref(
      ...
      "test",
      b_id,
      super::super::entity::RefType::SimilarTo,
      0.9,
    )
    .unwrap();
  store
...
fn temp_path(name: &str) -> (FileGuard, PathBuf) {
  let path = std::env::temp_dir().join(format!("rb_store_{}_{}.rdb", name, std::process::id()));
...
fn test_cross_refs_persist_file_mode() {
  ...
  let node = UnifiedEntity::graph_node(store.next_entity_id(), "host", "asset", HashMap::new());
  let node_id = store.insert_auto("graph", node).unwrap();

  let vector = UnifiedEntity::vector(store.next_entity_id(), "embeddings", vec![0.1, 0.2, 0.3]);
  ...
  store
    .add_cross_ref(
      ...
      "graph",
      node_id,
      super::super::entity::RefType::RowToNode,
      1.0,
    )
    .unwrap();
  store
    .add_cross_ref(
      ...
      "embeddings",
      vector_id,
      super::super::entity::RefType::NodeToVector,
      1.0,
    )
    .unwrap();
  ...
  let loaded = super::core::UnifiedStore::load_from_file(&path).unwrap();
  let refs = loaded.get_refs_from(row_id);
  assert!(refs.iter().any(|(id, kind, coll)| {
    *id == node_id && *kind == super::super::entity::RefType::RowToNode && coll == "graph"
  }));

  let graph_refs = loaded.get_refs_from(node_id);
  assert!(graph_refs.iter().any(|(id, kind, coll)| {
    *id == vector_id && *kind == super::super::entity::RefType::NodeToVector && coll == "embeddings"
  }));
...
fn test_cross_refs_persist_paged_mode() {
  ...
  let loaded = super::core::UnifiedStore::open(&path).unwrap();
  let refs = loaded.get_refs_from(row_id);
  assert!(refs.iter().any(|(id, kind, coll)| {
    *id == node_id && *kind == super::super::entity::RefType::RowToNode && coll == "graph"
  }));
...
fn test_global_ids_unique_across_collections() {
  ...
  let entity_a =
    UnifiedEntity::table_row(super::super::entity::EntityId::new(0), "alpha", 1, vec![]);
  let entity_b =
    UnifiedEntity::table_row(super::super::entity::EntityId::new(0), "beta", 1, vec![]);
