  ...
  for col_name in &collections {
    if let Some(manager) = store.get_collection(col_name) {
      ...
      for entity in entities {
        ...
        let similarity = calculate_entity_similarity(&entity, &query.vector, &query.embedding_slot);
...
pub fn execute_hybrid_query(
  ...
  let start = Instant::now();
  let mut entity_scores: HashMap<EntityId, (UnifiedEntity, f32, MatchComponents)> = HashMap::new();
  let mut scanned = 0;
  ...
  match start_phase {
    ...
    JoinPhase::TableStart { ref table } => {
      ...
      if let Some(manager) = store.get_collection(table) {
        let entities = manager.query_all(|e| matches!(&e.kind, EntityKind::TableRow { .. }));
                ...
                if seen.insert(entity.id) {
                  ...
                  let entry = new_results.entry(entity.id).or_insert_with(|| {
                    ...
                    CrossModalMatch {
                      ...
                      graph_score: current.graph_score + 1.0 / (*depth as f32 + 1.0),
            ...
            if let Some(current) = results.get(id) {
              ...
              for table_entity in &table_entities {
                ...
                if matches {
                  let entry =
                    new_results
                      .entry(table_entity.id)
                      .or_insert_with(|| CrossModalMatch {
              ...
              for col_name in store.list_collections() {
                if let Some(manager) = store.get_collection(&col_name) {
                  ...
                  for entity in entities {
                    if entity.id != *id {
                      scanned += 1;
                      let sim = calculate_entity_similarity(&entity, &emb.vector, &None);
                      if sim > 0.3 {
                        scored.push((entity.id, entity, sim));
                      }
                    }
                  ...
                  for (eid, entity, sim) in scored {
                    let entry = new_results.entry(eid).or_insert_with(|| CrossModalMatch {
                      entity,
                      vector_score: sim,
                      graph_score: current.graph_score,
                      table_score: current.table_score,
