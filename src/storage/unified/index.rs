...
impl InvertedIndex {
  ...
  pub fn search(&self, query: &str, limit: usize) -> Vec<TextSearchResult> {
    ...
    for postings in term_postings.iter().skip(1) {
      ...
      let entities_in_term: HashSet<EntityId> = postings.iter().map(|p| p.entity_id).collect();
...
impl IntegratedIndexManager {
  ...
  pub fn query_metadata(&self, key: &str, filter: MetadataQueryFilter) -> Vec<EntityId> {
    ...
    match filter {
      ...
      MetadataQueryFilter::Contains(ref substring) => storage.filter_string_prefix(key, substring),
...
mod tests {
  ...
  fn test_graph_adjacency_label_filter() {
    ...
    let neighbors = index.get_neighbors(EntityId(1), EdgeDirection::Outgoing, Some("WORKS_WITH"));
    assert_eq!(neighbors.len(), 1);
