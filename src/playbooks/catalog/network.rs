...
pub fn network_pivot() -> Playbook {
  Playbook::new("network-pivot", "Network Pivoting and Lateral Movement")
    ...
    .add_failed_control(
      FailedControl::new("Network Segmentation", "Insufficient network isolation").with_fix(
        "Implement micro-segmentation, restrict inter-VLAN traffic, monitor lateral movement",
      ),
    )
}
...
pub fn service_exploitation() -> Playbook {
  Playbook::new("service-exploitation", "Common Service Exploitation")
    ...
    .add_failed_control(
      FailedControl::new("Service Hardening", "Vulnerable service configurations")
        .with_fix("Patch services, disable legacy protocols, enforce strong auth, use firewalls"),
    )
}
...
pub fn network_mapping() -> Playbook {
  Playbook::new("network-mapping", "Internal Network Mapping")
    ...
    .with_objective(
      "Create comprehensive internal network diagram with all discoverable hosts and services",
    )
    .for_target(TargetType::Network)
    .for_target(TargetType::Internal)
    .for_os(TargetOS::Any)
    ...
    .add_failed_control(
      FailedControl::new(
        "Network Segmentation",
        "Flat network allows broad discovery",
      )
