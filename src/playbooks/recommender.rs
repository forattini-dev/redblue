...
impl PlaybookRecommender {
  ...
  fn vuln_matches_technique(&self, vuln: &VulnerabilityRecord, technique: &str) -> bool {
    ...
    match technique {
      ...
      "T1068" => {
        desc_lower.contains("privilege escalation") || desc_lower.contains("local privilege")
      }
      ...
      "T1552" => desc_lower.contains("unsecured credentials") || desc_lower.contains("plaintext"),
...
mod tests {
  ...
  fn test_empty_findings_returns_no_recommendations() {
    ...
    assert!(
      result.recommendations.is_empty() || result.recommendations.iter().all(|r| r.score < 20)
    );
  }
