...
use crate::scripts::{ScriptRunner, ScriptContext, builtin};
...
impl PlaybookExecutor {
    ...
    /// Execute a playbook
    pub fn execute(&self, playbook: &Playbook, context: &mut PlaybookContext) -> PlaybookExecutionResult {
        ...
        for step in &playbook.steps {
            ...
            if !step.depends_on.is_empty() {
                let all_deps_met = step.depends_on.iter().all(|dep_num| {
                    result.step_results.iter().any(|r| r.step_number == *dep_num && r.success)
                });
    ...
    fn execute_step(&self, step: &PlaybookStep, context: &mut PlaybookContext) -> StepExecutionResult {
        ...
        for script_id in &step.scripts {
            if let Some(script) = builtin::get_script(script_id) {
                ...
                if !script_res.success {
                    result.output.push(format!("Script {} failed or found nothing", script_id));
                } else {
                    result.output.push(format!("Script {} completed successfully", script_id));
                    any_success = true;
                }
            } else {
                 result.output.push(format!("Script {} not found", script_id));
                ...
                match Command::new(cmd).args(args).output() {
                    Ok(output) => {
                         ...
                         if output.status.success() {
                             ...
                             result.output.push(format!("Command failed: {}\nStderr: {}", cmd_str, stderr));
                             any_failure = true;
                         }
                    },
                    Err(e) => {
                        result.output.push(format!("Failed to execute command '{}': {}", cmd_str, e));
                        any_failure = true;
                    }
                }
            }
...
mod tests {
    use super::*;
    use crate::playbooks::{Playbook, PlaybookStep, PlaybookPhase, PreCondition};
    ...
    fn test_executor_dry_run() {
        ...
        playbook.steps.push(
            PlaybookStep::new(1, PlaybookPhase::Recon, "Test Step")
