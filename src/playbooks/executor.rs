...
impl PlaybookExecutor {
    ...
    fn evaluate_condition(
        ...
        match condition {
            ...
            StepCondition::OnEvidence(evidence_type) => {
                self.collected_evidence.contains(evidence_type)
            }
...
mod tests {
    ...
    fn test_split_command_line_single_quotes() {
        let args =
            PlaybookExecutor::split_command_line("echo 'hello world'").expect("parse failed");
...
mod tests {
    ...
    fn test_executor_records_actions() {
        let mut playbook = Playbook::new("test", "Test");
        playbook.steps.push(
            PlaybookStep::new(1, PlaybookPhase::Discovery, "Scan Step").with_command("echo scan"),
        );
        playbook.steps.push(
            PlaybookStep::new(2, PlaybookPhase::Collection, "Enum Step").with_command("echo enum"),
        );
    ...
    fn test_assertion_evaluation_greater_than() {
        ...
        playbook = playbook.add_assertion(
            Assertion::new("At least one step", "steps_completed").greater_than("0"),
        );
