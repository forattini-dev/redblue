...
- [ ] 2.6 Track time-to-first-byte separately from total response time and persist both in stats/livesnapshot.
- [ ] 2.7 Treat body read timeouts as warnings/errors instead of successful samples.
- [ ] 2.4 Add basic unit/integration coverage for protocol selection.

## 3. Validation
- [ ] 3.1 Run `cargo check` and `cargo test` for the touched crates. *(cargo check passes; `cargo test` currently fails in existing HTTP/2/QPACK tests — see notes)*
